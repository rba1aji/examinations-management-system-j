...
import server.rba1aji.academicmanagementsystem.exams.management.sqlqueries.MarkSQL;
...
public class MarkRepo implements IMarkRepo {
    ...
    public void update(Mark mark) {
        int count = jdbcTemplate.queryForObject(MarkSQL.SQL_MARKS_COUNT_BY_STUDENTID_EXAMID_COURSEID,
                new Object[]{mark.getStudentid(), mark.getExamid(), mark.getCourseid()}, Integer.class);
        jdbcTemplate.update(con -> {
            PreparedStatement ps;
            if (count == 0)
                ps = con.prepareStatement(MarkSQL.SQL_MARKS_INSERT);
            else
                ps = con.prepareStatement(MarkSQL.SQL_MARKS_UPDATE);
    ...
    public List<Mark> findByBatchidExamidCourseid(Integer batchid, Integer examid, String courseid) {
        return jdbcTemplate.query(MarkSQL.SQL_MARKS_GET_BY_BATCHID_EXAMID_COURSEID, new Object[]{batchid, examid, courseid}, markRowMapper);
    }

    @Override
    public List<Mark> findByStudentidAndExamid(Long studentid, Integer examid) {
        return jdbcTemplate.query(MarkSQL.SQL_MARKS_FINDBY_STUDENTID_EXAMID, new Object[]{studentid, examid}, markRowMapper);
    }
